...
// Card rarity types
type CardRarity = "basic" | "common" | "rare" | "epic" | "legendary" | "goat"
...
function getScoreForRarity(rarity: CardRarity): number {
  switch (rarity) {
    ...
    case "basic":
      return 2 // Basic-Karten geben 2 Punkte
    default:
      return 0
  }
}
...
export async function drawCards(walletAddress: string, packType: string, count = 1, hasPremiumPass = false) {
  try {
    ...
    const basicCards = availableCards.filter((card) => card.rarity === "basic")
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (isLegendary) {
        ...
        } else if (random < 30.35 + 26) {
          rating = Math.floor(Math.random() * 5) + 80; // 80–84
        } else if (random < 30.35 + 26 + 14) {
          rating = 85;
        } else if (random < 30.35 + 26 + 14 + 11.5) {
          rating = 86;
        } else if (random < 30.35 + 26 + 14 + 11.5 + 10) {
          rating = 87;
        } else if (random < 30.35 + 26 + 14 + 11.5 + 10 + 6.75) {
          rating = 88;
        } else if (random < 30.35 + 26 + 14 + 11.5 + 10 + 6.75 + 1.15) {
          rating = 89;
  ...
  if (packType === "legendary") {
    ...
    if (hasPremiumPass) {
      ...
      if (hasDropRateBoost && legendaryBonus > 0) {
        ...
        basicChance = 10 - legendaryBonus
        // Ensure basic doesn't go below 0
        if (basicChance < 0) {
          basicChance = 0
        }
      }
      ...
      if (random < legendaryChance + epicChance) return "epic"
      if (random < legendaryChance + epicChance + rareChance) return "rare"
      if (random < legendaryChance + epicChance + rareChance + commonChance) return "common"
      return "basic"
    } else {
